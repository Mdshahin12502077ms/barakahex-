        ...
        <div class="col-lg-8">
            ...
            <div class="bg-white redious-border p-20 p-sm-30 mb-4">
                ...
                <div class="row g-3 py-2 border-top">
                    ...
                    <div class="col-sm-6 col-md-3">
                        ...
                        <div class="fw-bold text-success font-16">{{ setting('default_currency') }} {{
                            number_format($parcel->price, 2) }}</div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                    ...
                    <div class="col-sm-6 col-md-3">
                        ...
                        <div class="fw-bold">{{ setting('default_currency') }} {{
                            number_format($parcel->total_delivery_charge, 2) }}</div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                    ...
                    <div class="col-sm-6 col-md-3">
                        ...
                        <div>{{ $parcel->delivery_date ? date('d M Y', strtotime($parcel->delivery_date)) : 'N/A' }}
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white redious-border p-20 p-sm-30 mb-4 border border-info">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-sm-6 col-md-6">
                        ...
                        <div class="fw-bold text-muted text-decoration-line-through">{{ setting('default_currency') }}
                            {{ number_format($parcel->price_before_delivery, 2) }}</div>
                    </div>
                    <div class="col-sm-6 col-md-6">
                ...
                <div class="timeline-wrapper">
                    ...
                    <div class="d-flex mb-4 position-relative">
                        <div class="me-3 text-center" style="min-width: 40px;">
                            <div class="badge rounded-circle bg-primary text-white p-2 d-inline-flex align-items-center justify-content-center"
                        ...
                        <div class="flex-grow-1 bg-light p-3 rounded">
                            <div class="d-flex justify-content-between align-items-center flex-wrap">
                                ...
                                <small class="text-muted">{{ date('d M Y, h:i A', strtotime($event->created_at))
                                    }}</small>
                            </div>
            ...
            <div class="bg-white redious-border p-20 p-sm-30 mb-4">
                <h5 class="fw-bold text-dark mb-3">
                    <i class="las la-shield-alt font-20 text-primary"></i> {{ __('otp_verification_history') ?? 'OTP
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <span class="badge bg-success"><i class="las la-check-circle"></i> {{ __('success')
                                        }}</span>
                                    @elseif($log->action == 'wrong_attempt')
                                    <span class="badge bg-warning text-dark"><i class="las la-exclamation-circle"></i>
                                        {{ __('wrong_attempt') ?? 'Wrong Attempt' }}</span>
                                    @elseif($log->action == 'locked')
                                    <span class="badge bg-danger"><i class="las la-lock"></i> {{ __('locked') }}</span>
                                    @elseif($log->action == 'expired')
                                    <span class="badge bg-secondary"><i class="las la-clock"></i> {{ __('expired')
                                        }}</span>
                                    @elseif($log->action == 'resend')
                                    <span class="badge bg-info text-white"><i class="las la-redo"></i> {{ __('resend')
                                        }}</span>
                                    @else
                                    <span class="badge bg-primary"><i class="las la-paper-plane"></i> {{ __('generated')
                                        }}</span>
                                    @endif
                                </td>
                                <td>
        ...
        <div class="col-lg-4">
            ...
            <div class="bg-white redious-border p-20 p-sm-30 mb-4">
                ...
                <div class="mb-3">
                    ...
                    <a href="tel:{{ $parcel->customer_phone_number }}"
                ...
                <div class="mb-3">
                    ...
                    <a href="https://www.google.com/maps/dir/?api=1&destination={{ urlencode($parcel->customer_address) }}"
                        target="_blank" class="btn btn-danger btn-sm w-100 py-2">
            ...
            <div class="bg-white redious-border p-20 p-sm-30 mb-4">
                ...
                <div class="mb-2">
                    <div class="fw-bold text-dark">{{ $parcel->shop->name ?? ($parcel->merchant->company_name ?? 'N/A')
                        }}</div>
                    <small class="text-muted">{{ $parcel->merchant->user->first_name ?? '' }} {{
                        $parcel->merchant->user->last_name ?? '' }}</small>
                </div>
                @php
                $mPhone = $parcel->shop->contact_number ?? ($parcel->merchant->phone_number ??
                ($parcel->merchant->user->phone_number ?? ''));
                @endphp
                @if($mPhone)
                <div class="mb-3">
            ...
            @php
                ...
                $deliveryMan = $user->deliveryMan ?? \App\Models\DeliveryMan::where('user_id', $user->id)->first();

                $isPickupAction = in_array($parcel->status, ['pickup_assigned', 'pickup-assigned', 'pickup_re_schedule', 're-schedule-pickup']);
                $isDeliveryAction = in_array($parcel->status, ['delivery-assigned', 're-schedule-delivery', 'processing', 'out-for-delivery']) && $parcel->delivery_man_id == $deliveryMan->id;
                $isOtpAction = in_array($parcel->status, ['delivered', 'partially-delivered']) && !empty($parcel->otp) && $parcel->delivery_man_id == $deliveryMan->id;
                
                $hasAnyAction = ($isPickupAction || $isDeliveryAction || $isOtpAction) && !in_array($parcel->status, ['delivered-and-verified', 'cancel', 'cancelled']);
            @endphp
            ...
            @if($hasAnyAction)
                <div class="bg-white redious-border p-20 p-sm-30 mb-4">
                    ...
                    <div class="d-grid gap-2">
                        @if($isPickupAction)
                            {{-- Pickup Actions --}}
                            <form action="{{ route('deliveryman.pickup.received', $parcel->id) }}" method="POST"
                                class="confirm-form" data-title="{{ __('are_you_sure') }}"
                            ...
                            <button type="button" class="btn btn-outline-warning w-100 py-2" data-bs-toggle="modal"
                                data-bs-target="#reschedulePickupModal">
                            ...
                            <button type="button" class="btn btn-outline-danger w-100 py-2" data-bs-toggle="modal"
                                data-bs-target="#cancelPickupModal">
                                <i class="las la-times-circle"></i> {{ __('cancel_pickup') }}
                            </button>
                        @elseif($isDeliveryAction)
                            {{-- Delivery Actions --}}
                            ...
                            <button type="button" class="btn btn-info btn-lg text-white w-100 py-2" data-bs-toggle="modal"
                                data-bs-target="#partialDeliveryModal">
                                <i class="las la-boxes"></i> {{ __('partially-delivered') }}
                            </button>
                            ...
                            <button type="button" class="btn btn-outline-danger w-100 py-2" data-bs-toggle="modal"
                                data-bs-target="#cancelDeliveryModal">
                                <i class="las la-times-circle"></i> {{ __('cancel_delivery') }}
                            </button>
                        @elseif($isOtpAction)
                            {{-- OTP Verification Action --}}
                            <button type="button" class="btn btn-warning btn-lg w-100 py-2 text-dark" data-bs-toggle="modal"
                                data-bs-target="#detailOtpModal">
                    ...
                    <div class="modal-content text-start">
                        <form action="{{ route('deliveryman.pickup.reschedule', $parcel->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label">{{ __('new_pickup_date') }} <span
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="note" class="form-control" rows="3"
                                        placeholder="{{ __('reason_for_rescheduling') }}"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('close')
                                    }}</button>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content text-start">
                        <form action="{{ route('deliveryman.pickup.cancel', $parcel->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label">{{ __('cancellation_reason') }} <span
                                            class="text-danger">*</span></label>
                                    <textarea name="note" class="form-control" rows="3" required
                                        placeholder="{{ __('enter_why_pickup_is_cancelled') }}"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('close')
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content text-start">
                        <form action="{{ route('deliveryman.reschedule', $parcel->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label class="form-label">{{ __('new_delivery_date') }} <span
                                            class="text-danger">*</span></label>
                                ...
                                <div class="mb-3">
                                    <label class="form-label">{{ __('reason') }} <span
                                            class="text-danger">*</span></label>
                                    <select name="note" class="form-select reason-select" required
                                ...
                                <div class="mb-3 custom-note-container d-none">
                                    <label class="form-label">{{ __('specify_other_reason') }} <span
                                            class="text-danger">*</span></label>
                                    <textarea name="custom_note" class="form-control" rows="3"
                                        placeholder="{{ __('enter_custom_reason') }}"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('close')
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content text-start">
                        <form action="{{ route('deliveryman.partial.delivered', $parcel->id) }}" method="POST">
                            @csrf
                            <div class="modal-header">
                                <h5 class="modal-title text-info"><i class="las la-boxes"></i> {{
                                    __('partially-delivered') }} - #{{ $parcel->parcel_no }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <div class="alert alert-info py-2 small mb-3">
                                    <strong>{{ __('total_order_amount') }}:</strong> {{ setting('default_currency') }}
                                ...
                                <div class="row">
                                    <div class="col-6 mb-3">
                                        <label class="form-label">{{ __('delivered_quantity') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="number" min="1" step="any" name="delivered_quantity"
                                            class="form-control" value="1" required
                                            placeholder="{{ __('delivered_quantity') }}">
                                    </div>
                                    <div class="col-6 mb-3">
                                        <label class="form-label">{{ __('return_quantity') }}</label>
                                        <input type="number" min="0" step="any" name="return_quantity"
                                            class="form-control" value="0" placeholder="{{ __('return_quantity') }}">
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-6 mb-3">
                                        <label class="form-label">{{ __('collected_cod_amount') }} <span
                                                class="text-danger">*</span></label>
                                        <input type="number" step="any" min="0" name="cod" class="form-control"
                                            value="{{ $parcel->price }}" required placeholder="{{ __('cod') }}">
                                    </div>
                                    <div class="col-6 mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="note" class="form-control" rows="2"
                                        placeholder="{{ __('remarks_or_reason') }}"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('close')
                                    }}</button>
                                <button type="submit" class="btn btn-info text-white">{{ __('confirm_partial_delivery')
                                    }}</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="modal-content text-start">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="alert alert-danger py-2 small mb-3">
                                <i class="las la-lock font-18"></i> <strong>{{ __('otp_locked') }}:</strong> {{
                                __('max_wrong_otp_attempts_reached_please_resend') }}
                            </div>
                            @elseif(!empty($parcel->otp_attempts) && $parcel->otp_attempts > 0)
                            <div
                                class="alert alert-warning py-2 small mb-3 d-flex justify-content-between align-items-center">
                                <span><i class="las la-exclamation-circle"></i> {{ __('wrong_attempts') }}: <strong>{{
                                        $parcel->otp_attempts }}/3</strong></span>
                                <span class="badge bg-warning text-dark">{{ 3 - $parcel->otp_attempts }} {{
                                    __('attempts_left') }}</span>
                            </div>
                            @endif
                            ...
                            @else
                            <div
                                class="alert alert-info py-2 small mb-3 d-flex justify-content-between align-items-center">
                                <span><i class="las la-clock"></i> {{ __('otp_valid_until') }}: <strong>{{
                                        \Carbon\Carbon::parse($parcel->otp_expired_at)->format('h:i:s A')
                                        }}</strong></span>
                                <span class="badge bg-primary">{{
                                    \Carbon\Carbon::parse($parcel->otp_expired_at)->diffForHumans(['parts' => 1])
                                    }}</span>
                            </div>
                            @endif
                            @endif

                            <form action="{{ route('deliveryman.verify.otp', $parcel->id) }}" method="POST"
                                id="detailOtpVerifyForm">
                                @csrf
                                <p class="text-muted small mb-3">{{ __('enter_4_digit_otp_sent_to_customer') }}</p>
                                <div class="mb-3">
                                    <label class="form-label">{{ __('otp_code') }} <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="otp"
                                        class="form-control form-control-lg text-center fw-bold" required maxlength="10"
                                        placeholder="1234" {{ ($parcel->otp_attempts >= 3) ? 'disabled' : '' }}>
                                </div>
                            </form>
                        </div>
                        <div class="modal-footer d-flex justify-content-between">
                            <form action="{{ route('deliveryman.resend.otp', $parcel->id) }}" method="POST"
                                class="d-inline">
                                @csrf
                            ...
                            <div class="d-flex gap-2">
                                ...
                                @if(($parcel->otp_attempts ?? 0) < 3) <button type="button" class="btn btn-success"
                                    onclick="document.getElementById('detailOtpVerifyForm').submit();">{{
                                    __('confirm_verification') }}</button>
                                    @else
                                    <button type="button" class="btn btn-secondary" disabled
                                        title="{{ __('max_wrong_otp_attempts_reached_please_resend') }}">{{
                                        __('confirm_verification') }}</button>
                                    @endif
                            </div>
                        </div>
